...
public ArrayList<Historique> getTousHistorique() {
		...
		// connexion è„™ç‚‰è„—é©´è„—é™† la base de donnè„™æ¼�èŠ’éˆ§îƒ½ï¿½æº�â‘©å�“è—´s
		try {
			...
			rs = ps.executeQuery();
			// on parcourt les lignes du rè„™æ¼�èŠ’éˆ§îƒ½ï¿½æº�å�¢ç«¨ltat
			while (rs.next()){
...
public List<Historique> getHistoriqueClient( int idClient) {
	...
	// connexion 猫鈥炩劉莽鈥氣�懊ㄢ�炩�斆┞┞疵ㄢ�炩�斆┾劉鈥� la base de donn猫鈥炩劉忙录锟矫ㄅ犫�櫭┧喡捖矫柯矫β猴拷芒鈥樎┟ワ拷鈥溍ㄢ�斅�?
	try {

		con = DriverManager.getConnection(URL, LOGIN, PASS);
		ps = con.prepareStatement("SELECT * FROM historique WHERE  idClient= ? ORDER BY date ASC");
		ps.setInt(1, idClient);
		
		// on ex猫鈥炩劉忙录锟矫ㄅ犫�櫭┧喡捖矫柯矫β猴拷卯鈥÷访ε嘎德瑃e la requ猫鈥炩劉忙录锟矫ㄢ�灺Ｃㄢ�扳劉猫鈥炩�斆λ喡�?
		rs = ps.executeQuery();
